...
- **響應式全螢幕佈局 (Responsive Full-Viewport Layout)**：
  - **手機版 (Mobile)**：採用 `100dvh` 鎖定整個網頁的視窗高度，禁止 `body` 上下滾動。操作按鈕（翻頁、記住了、還不熟）永遠固定且釘選於畫面底部，方便大拇指操作。單字卡主體採用 `flex-grow` 彈性延展，若卡片背面內容過長，僅允許在卡片「內部」進行滾動。確保獲得如同 Native App 般流暢的體驗。
  - **平板與電腦版 (Tablet / PC)**：當螢幕寬度達平板以上（如 `768px`），自動透過 Media Queries 切換為「雙欄式佈局 (Split Layout)」。左側為固定的 Dashboard（儀表板與選單），右側為寬敞的單字卡學習區與按鈕，充分利用大螢幕的空間。
- **高級質感設計**：
